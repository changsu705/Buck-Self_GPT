...
/// <summary>
/// [총기 시각효과] 리볼버 실린더를 "꺼내서(OPEN) → 유지(HOLD) → 넣는(CLOSE)"
/// 3단계 프리뷰 '연출'을 담당합니다. (데이터 조작 X)
/// 
/// - 'PlayOnce()'로 코루틴('CoPeek') 실행
/// - PeekCard.cs 또는 R키 입력, 총기 픽업 시 호출됨
/// - 'bulletSlots' 배열의 가시성을 자동으로 제어
/// </summary>
public class RevolverCylinderPeek : MonoBehaviour
{
    ...
    /// <summary>
    /// [Public] 외부에서 실린더 프리뷰를 1회 재생하도록 호출하는 함수입니다.
    /// (예: PeekCard.cs, RevolverTurnPossession.cs)
    /// </summary>
    ...
    private System.Collections.IEnumerator CoPeek(Action onComplete = null)
    {
        ...
        {
            // (스핀 연출은 중간에만 들어가고) 최종적으론 원본 각도로 복귀
            closeTargetRot = snapToOriginAfterClose
                ? _originLocalRot
                : _originLocalRot * Quaternion.AngleAxis(spinDegClose, AxisVec(spinAxis));
        }
        ...
        onComplete?.Invoke(); // 완료 콜백이 있다면 호출
    }
