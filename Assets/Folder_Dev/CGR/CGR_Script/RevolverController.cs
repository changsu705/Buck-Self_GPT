...
/// 
/// - '총을 든 상태'일 때('RevolverTurnPossession'이 'SetInputEnabled(true)' 호출)만 작동합니다.
/// - 유효한 타겟('hitMask')을 조준하고 발사 키('fireKey')를 누르면 'OnAimRequest' 이벤트를 발생시킵니다.
/// - 'ExecuteShot' 코루틴이 실제 발사(리코일)를 처리합니다.
/// </summary>
/// 
...
public class RevolverController : MonoBehaviour
{
    ...
    /// <summary>
    /// 발사 등 총기 액션이 완료되었을 때 발생 (현재는 사용되지 않음)
    /// </summary>
    public event Action OnActionComplete;

    /// <summary>
    /// 유효 타겟 조준 후 발사 키를 눌렀을 때 발생 (조준 연출 트리거)
    /// 'RevolverTurnPossession'이 이 이벤트를 구독하여 CoAimAndFire를 실행합니다.
    /// </summary>
    public event Action<Transform> OnAimRequest;

    // ────────────────────────────── 내부 상태 ──────────────────────────────

    // 현재 총기 입력이 활성화되었는지 (RevolverTurnPossession이 제어)
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(fireKey))
        {
            ...
            // 1. 카메라 중앙(십자선)에서 Raycast 발사 및 유효 타겟(hitMask) 명중 확인
    ...
    /// <summary>
    /// 'RevolverTurnPossession'이 이 함수를 호출하여 총기 입력을 활성화/비활성화합니다.
    /// </summary>
    public void SetInputEnabled(bool enabled) => _inputEnabled = enabled;

    /// <summary>
    /// 총 발사 로직(리코일)을 실행하는 '코루틴'입니다.
    /// 'RevolverTurnPossession'이 이 코루틴이 끝날 때까지 'yield return'으로 기다립니다.
    /// </summary>
    public IEnumerator ExecuteShot() // 👈 void에서 IEnumerator로 변경됨
    {
        // 발사 로그 (디버그용)
        // (실제 데미지 처리는 RevolverTurnPossession이 담당)
